...
<script>
/**
 * Classroom Floor Plan Generator (24 Students)
 * Configured for 6 groups of 4 in a 630x450cm space.
 * Language: Traditional Chinese (繁體中文)
 */

const { state, ui } = WH.createApp({
    title: "24人班級座位配置 (6組4人簇) - 優化尺寸限制",
    params: {
        showDimensions: { value: true, label: "顯示尺寸標註" },
        showLegend: { value: true, label: "顯示圖例" }
    }
});

// 1. Classroom & Furniture Constants (in cm)
const ROOM_W = 630;
const ROOM_D = 450;
const STUDENT_W = 75;
const STUDENT_D = 105;
const GROUP_COLS = 3;
const GROUP_ROWS = 2;
const FRONT_CLEARANCE = 30;
const SIDE_AISLE = 30;
const CENTER_AISLE = 120;

// Internal state for hover/selection
state.hoveredGroup = null;

// Helper to draw dimension line
function drawDimension(ctx, x1, y1, x2, y2, text, scale, offsetX = 0, offsetY = 0) {
    ...
    // Draw main arrow
    ctx.drawArrow(x1, y1, x2 - x1, y2 - y1, '--outline');
    ctx.drawArrow(x2, y2, x1 - x2, y1 - y2, '--outline');
...
WH.initCanvas('viz', (ctx) => {
    return ({ ctx, width, height, pointer }) => {
        // Calculate scaling (Padding 10%)
        const padding = 60;
        const availableW = width - padding * 2;
        const availableH = height - padding * 2;
        const scale = Math.min(availableW / ROOM_W, availableH / ROOM_D);
        
        // Center the room
        const offsetX = (width - ROOM_W * scale) / 2;
        const offsetY = (height - ROOM_D * scale) / 2;

        const toPx = (cm) => cm * scale;
        const toX = (cm) => offsetX + cm * scale;
        const toY = (cm) => offsetY + cm * scale;

        // 1. Draw Classroom Boundary
        ctx.fillStyle = WH.getColor('--surface-container-low');
        ctx.strokeStyle = WH.getColor('--on-surface-default');
        ctx.lineWidth = 4;
        ctx.strokeRect(toX(0), toY(0), toPx(ROOM_W), toPx(ROOM_D));
        ctx.fillRect(toX(0), toY(0), toPx(ROOM_W), toPx(ROOM_D));

        // 2. Front Area (Blackboard & Teacher)
        // Blackboard
        ctx.fillStyle = '#2d5a27'; // Classic dark green blackboard
        ctx.fillRect(toX(ROOM_W * 0.2), toY(-5), toPx(ROOM_W * 0.6), toPx(10));
        ctx.fillStyle = WH.getColor('--on-primary-container');
        ctx.font = 'bold 16px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('黑板 (Blackboard)', toX(ROOM_W / 2), toY(15));

        // Teacher Area
        const teacherDeskW = 120;
        const teacherDeskD = 60;
        const tx = toX(ROOM_W / 2 - teacherDeskW / 2);
        const ty = toY(10);
        ctx.fillStyle = WH.getColor('--surface-container-highest');
        ctx.strokeRect(tx, ty, toPx(teacherDeskW), toPx(teacherDeskD));
        ctx.fillRect(tx, ty, toPx(teacherDeskW), toPx(teacherDeskD));
        
        ctx.save();
        ctx.font = '24px "Google Symbols"';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('person', toX(ROOM_W / 2), toY(25));
        ctx.restore();
        ctx.drawTag('教師區', toX(ROOM_W / 2), toY(40));

        // 3. Draw Student Groups
        // Calculated Layout: [Side 30] [Group 150] [Center 120] [Group 150] [Group 150] [Side 30]
        const groupWidthCm = STUDENT_W * 2; // 150cm
        const groupDepthCm = STUDENT_D * 2; // 210cm
        
        const colPositions = [
            SIDE_AISLE, 
            SIDE_AISLE + groupWidthCm + CENTER_AISLE,
            SIDE_AISLE + groupWidthCm + CENTER_AISLE + groupWidthCm
        ];
        const rowPositions = [FRONT_CLEARANCE, FRONT_CLEARANCE + groupDepthCm];

        colPositions.forEach((gx, ci) => {
            rowPositions.forEach((gy, ri) => {
                const px = toX(gx);
                const py = toY(gy);
                const pw = toPx(groupWidthCm);
                const ph = toPx(groupDepthCm);

                // Group Background
                ctx.fillStyle = WH.transparent('--primary', 0.1);
                ctx.strokeStyle = WH.getColor('--primary');
                ctx.lineWidth = 1;
                ctx.fillRect(px, py, pw, ph);
                ctx.strokeRect(px, py, pw, ph);

                // Draw 4 Student Seats per Group
                for(let i=0; i<2; i++) {
                    for(let j=0; j<2; j++) {
                        const sx = px + toPx(i * STUDENT_W);
                        const sy = py + toPx(j * STUDENT_D);
                        const sw = toPx(STUDENT_W);
                        const sh = toPx(STUDENT_D);
                        ...
                        // Icon for student
                        ctx.save();
                        ctx.fillStyle = WH.getColor('--secondary');
                        ctx.font = '20px "Google Symbols"';
                        ctx.textAlign = 'center';
                        ctx.fillText('person', sx + sw/2, sy + sh/2);
                        ctx.restore();
                    }
                }

                // Group Label
                ctx.drawTag(`第 ${ci * 2 + ri + 1} 組`, px + pw/2, py + ph/2 + 20);
            });
        });
        ...
        ctx.fillRect(toX(ROOM_W - 5), toY(ROOM_D - 100), toPx(10), toPx(80));
        ctx.drawTag('門', toX(ROOM_W - 10), toY(ROOM_D - 60));

        // Windows (Left Wall)
        for(let i=0; i<3; i++) {
            ctx.fillStyle = WH.getColor('--chart-1');
            ctx.fillRect(toX(-5), toY(50 + i * 140), toPx(10), toPx(80));
        }
        ctx.drawTag('窗戶', toX(15), toY(190));

        // 5. Dimensions
        if (state.showDimensions) {
            // Room width
            drawDimension(ctx, toX(0), toY(-30), toX(ROOM_W), toY(-30), '教室寬 630cm', scale);
            // Room depth
            drawDimension(ctx, toX(-30), toY(0), toX(-30), toY(ROOM_D), '全長 450cm', scale);
            
            // Aisle dimensions
            drawDimension(ctx, toX(0), toY(ROOM_D + 20), toX(SIDE_AISLE), toY(ROOM_D + 20), '30', scale);
            drawDimension(ctx, toX(SIDE_AISLE), toY(ROOM_D + 20), toX(SIDE_AISLE + groupWidthCm), toY(ROOM_D + 20), '150', scale);
            drawDimension(ctx, toX(SIDE_AISLE + groupWidthCm), toY(ROOM_D + 20), toX(SIDE_AISLE + groupWidthCm + CENTER_AISLE), toY(ROOM_D + 20), '中央通道 120cm', scale);
            drawDimension(ctx, toX(SIDE_AISLE + groupWidthCm + CENTER_AISLE), toY(ROOM_D + 20), toX(ROOM_W - SIDE_AISLE), toY(ROOM_D + 20), '300 (2組)', scale);
            
            // Individual group detail (top-left group)
            drawDimension(ctx, toX(SIDE_AISLE + groupWidthCm + 10), toY(FRONT_CLEARANCE), toX(SIDE_AISLE + groupWidthCm + 10), toY(FRONT_CLEARANCE + groupDepthCm), '簇深 210cm', scale, 40);
        }

        // 6. Legend (Cockpit HUD replacement for standard legend)
        if (state.showLegend) {
            ui.setHUD([
                { label: "人數", value: "24人 (6組x4人)" },
                { label: "座位尺寸", value: "75x105 cm" },
                { label: "中央走道", value: "120 cm" },
                { label: "側走道", value: "30 cm" },
                { label: "空間坪數", value: "約 8.6 坪" }
            ]);
        } else {
            ui.setHUD([]);
        }

        // Pointer interactions (Show details)
        if (pointer.isDown) {
            ctx.drawTag('點擊拖移查看細節', pointer.x, pointer.y - 40);
        }
    };
});
</script></body>
