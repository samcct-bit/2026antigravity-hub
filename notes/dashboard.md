...
> 位置：`d:\antigravity\2026antigravity\notes\dashboard.md`
> 本日記錄：2026-06-17
...
- **2026-07-18**：🟢 **重製一日基礎班（S__23208083.jpg）新版課程海報**。
  - **海報底圖補丁羽化**：使用 Python PIL 對 `S__23208083.jpg` 進行局部的漸層遮蓋，開發出「不對稱漸層羽化」遮罩，遮蔽「研究班」與原中段小字，生成 `S__23208083_patched.jpg` 以防 ghost outline text，呈現無接縫的高畫質新底圖。
  - **新課程排版與 Playwright 渲染**：建立 HTML 模板並調整文字大小與行高，適配 A4 比例，填入 2026-07-26 (星期日) 的一日基礎班資訊（上午啟發性課程、下午手語表演），並使用 Playwright 自動化渲染出 1000px × 1415px 高清海報 `poster.png`。
